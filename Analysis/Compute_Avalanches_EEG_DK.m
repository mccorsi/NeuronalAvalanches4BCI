function Compute_Avalanches_EEG_DK(path_file_MI, path_file_Rest, subject_IDs, db_path, labels_DK, freq_band_def, z_thresh, fs, min_size_aval)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
nb_subj=size(Data_EEG_Baseline_DK, 1); % numeri di soggetti
nb_ROIs=size(Data_EEG_Baseline_DK{1,1}, 1); % x verificare il numero di ROIs
nbsamples =size(Data_EEG_Baseline_DK{1,1}, 2); % numero di campioni
ROIs = [1:nb_ROIs]; 
thresh = z_thresh;
...
for kk_subj=1:nb_subj % ogni soggetto
    ...
    notempty_MI_idx=find(~cellfun(@isempty,{Data_EEG_MI_DK{kk_subj,:}}));
    notempty_Baseline_idx=find(~cellfun(@isempty,{Data_EEG_Baseline_DK{kk_subj,:}}));
    nb_trials_tot=size(notempty_MI_idx,2) + size(notempty_Baseline_idx,2);

    % 0 x Baseline
    Dataset_EEG_Baseline_DK.data={Data_EEG_Baseline_DK{kk_subj,1:size(notempty_Baseline_idx,2)}};
    Dataset_EEG_Baseline_DK.condition(1:size({Data_EEG_Baseline_DK{kk_subj,1:size(notempty_Baseline_idx,2)}},2))=zeros(1,size({Data_EEG_Baseline_DK{kk_subj,1:size(notempty_Baseline_idx,2)}},2));

    % 1 x MI
    Dataset_EEG_MI_DK.data={Data_EEG_MI_DK{kk_subj,1:size(notempty_MI_idx,2) }};
    Dataset_EEG_MI_DK.condition(1:size({Data_EEG_MI_DK{kk_subj,1:size(notempty_MI_idx,2) }},2))=ones(1,size({Data_EEG_MI_DK{kk_subj,1:size(notempty_MI_idx,2) }},2)); 

    Dataset_EEG_DK.data=[Dataset_EEG_MI_DK.data,Dataset_EEG_Baseline_DK.data];
    Dataset_EEG_DK.condition=[Dataset_EEG_MI_DK.condition,Dataset_EEG_Baseline_DK.condition];

    %% calcolo delle "rasterplot" x ogni trial
    % concat MI & Baseline x calcolare il z-score
    data=[Dataset_EEG_DK.data{:}];
    ...
    for kk_freq=1:size(freq_band_def,1) % ogni banda di frequenza considerata
        for kk_zthresh=1:length(z_thresh) % ogni soglia x z-score  
            for kk_min_size_aval=1:size(min_size_aval)
                ...
                disp(strcat('%%%%%%%%%%%%%%%%%% EEG - DK - ',subject_IDs{kk_subj}, ' - ', num2str(freq_band_def{kk_freq}(1)), '-', num2str(freq_band_def{kk_freq}(2)),'Hz ',' - z_thresh:', num2str(z_thresh(kk_zthresh)), ' %%%%%%%%%%%%%%%%%%'));
    ...
    %                 aval_binned=cell(1,size(n_binning,2));
                ...
                val_empty.conditions=Dataset_EEG_DK.condition;
           ...
           % salva i risultati in a .mat file in caso x altre analisi
            ...
            save(strcat(db_path,'Avalanches_Analysis_ds_',subject_IDs{kk_subj} ,'_',num2str(freq_band_def{kk_freq}(1)), '_', num2str(freq_band_def{kk_freq}(2)),'_z_', num2str(z_thresh(kk_zthresh)), '_min_size_aval_',num2str(min_size_aval(kk_min_size_aval)),'_Sess4_EEG_DK.mat'),...
                'Dataset_EEG_DK',...
                'TM_out',...
                'TM_abs_Diff',...
                'TM_Diff',...
        ...
        %     clearvars Dataset_EEG_DK Dataset_EEG_Baseline_DK Dataset_EEG_MI_DK avalanches TM_out
            end
end
