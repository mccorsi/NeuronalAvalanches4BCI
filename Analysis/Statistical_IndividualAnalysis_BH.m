function Statistical_IndividualAnalysis_BH(filename_Avalanches_subj, filename_Avalanches_subj_file, fig_path, db_path, numperm, nb_ROIs_DK, p_thresh, cortex_15002V_MNI, idx_DK,n_binning)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Perform individual-level statistical analysis 
    % Authors: MCC
    % Date: 20/03/2022
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; %clc;
...
data_3D=cat(3, TM_out{n_binning}{:}); % considers all possible binning

nb_trials_tot=size(data_3D,3);
val_empt_MI=~val_empty.values(1:size(notempty_MI_idx,2));
MI=data_3D(:,:,val_empt_MI);
Node_MI=squeeze(sum(MI,2)); % nb_ROIs x nb_trials

Base=data_3D(:,:,find(val_empty.conditions==0,1):nb_trials_tot);
...
output_cat=cat(2,output(:,:,1),output(:,:,2));
...
colorbar
caxis([0 0.05])
filename=strcat(fig_path,'MI_vs_Rest_Edges_diff_BH_corrected_p_thresh_', num2str(p_thresh), '_',filename_Avalanches_subj_file);
saveas(gcf,strcat(filename,'.pdf'));


save(strcat(db_path,'Stat_Analysis_Indiv_',filename_Avalanches_subj_file,'_BH_p_thresh_',num2str(p_thresh),'.mat'),...
