...
path_file_MI=strcat(db_path,'Data_MEG_MI_Destrieux_V2.mat');
path_file_Rest=strcat(db_path,'Data_MEG_Baseline_Destrieux_V2.mat');

Compute_Avalanches_MEG_Destrieux(path_file_MI, path_file_Rest, subject_IDs, db_path, labels_Destrieux, freq_band_def, z_thresh, fs, min_size_aval)
    ...
    for kk_zthresh=1:length(z_thresh) % ogni soglia x z-score 
        ...
        %% TODO : add loop sul binning
            ...
            for kk_subj=1:nb_subj % ogni soggetto
                filename_Avalanches_subj=strcat(db_path,'Avalanches_Analysis_ds_',subject_IDs{kk_subj} ,'_',num2str(freq_band_def{kk_freq}(1)), '_', num2str(freq_band_def{kk_freq}(2)),'_z_', num2str(z_thresh), '_min_size_aval_',num2str(min_size_aval(kk_min_size_aval)),'_Sess4_MEG_Destrieux.mat');
                ...
                disp(strcat('%%%%%%%%%%%%%%%%%% Individual stat analysis - MEG - Destrieux - ',subject_IDs{kk_subj}, ' - ', num2str(freq_band_def{kk_freq}(1)), '-', num2str(freq_band_def{kk_freq}(2)),'Hz ',' - z_thresh:', num2str(z_thresh), ' - p_thresh:,' , num2str(p_thresh_stat),'%%%%%%%%%%%%%%%%%%'));
                % individual-level
...
%         signif_abs_cmp_bhfdr=mafdr(p_vect,'BHFDR','true');
        ...
        axis square
        filename=strcat(fig_path,'GroupAnalysis_Edge_Based_Reliable_Edges_BH_abs_diff_Sess4_MEG_Destrieux_p_thresh_', num2str(0.05), '_',num2str(freq_band_def{kk_freq}(1)), '_', num2str(freq_band_def{kk_freq}(2)),'_z_', num2str(z_thresh),'_binning_',num2str(n_binning(kk_binning)),'_min_size_aval_',num2str(min_size_aval(kk_min_size_aval)), '_Sess4_', modality, '_',atlas, '.png');
        print('-f2', filename, '-dpng','-r600');

        DoMyViz_node_rel(cortex_15002V_MNI, idx_Destrieux, node_abs_cmp, strcat(fig_path,'GroupAnalysis_Reliable_Edge_Based_Nodes_BH_abs_diff_Sess4_MEG_Destrieux_p_thresh_', num2str(0.05),'_',num2str(freq_band_def{kk_freq}(1)), '_', num2str(freq_band_def{kk_freq}(2)),'_z_', num2str(z_thresh), '_binning_',num2str(n_binning(kk_binning)),'_min_size_aval_',num2str(min_size_aval(kk_min_size_aval)),'_Sess4_', modality, '_',atlas), 'Sess4_MEG_Destrieux')
        DoMyViz_node_rel(cortex_15002V_MNI, idx_Destrieux, node_cmp, strcat(fig_path,'GroupAnalysis_Reliable_Edge_Based_Nodes_BH_diff_Sess4_MEG_Destrieux_p_thresh_', num2str(0.05),'_',num2str(freq_band_def{kk_freq}(1)), '_', num2str(freq_band_def{kk_freq}(2)),'_z_', num2str(z_thresh), '_binning_',num2str(n_binning(kk_binning)),'_min_size_aval_',num2str(min_size_aval(kk_min_size_aval)),'_Sess4_', modality, '_',atlas), 'Sess4_MEG_Destrieux')
            end
        end
   end
end
